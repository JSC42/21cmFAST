...
#pragma omp for
                ...
                if (!log10_Mturnover_unfiltered || !log10_Mturnover_filtered || !log10_Mturnover_MINI_unfiltered || !log10_Mturnover_MINI_filtered)
                { // || !Mcrit_RE_grid || !Mcrit_LW_grid)
                ...
                else
                {
                    box->mean_f_coll_MINI = previous_ionize_box->mean_f_coll_MINI +
                                            Nion_General_MINI(redshift, M_MIN, Mturnover_MINI, Mcrit_atom, astro_params->ALPHA_STAR_MINI,
                                                              astro_params->ALPHA_ESC, astro_params->F_STAR7_MINI, astro_params->F_ESC7_MINI, Mlim_Fstar_MINI, Mlim_Fesc_MINI) -
...
#pragma omp for reduction(max : log10Mturn_max, log10Mturn_max_MINI) reduction(min : log10Mturn_min, log10Mturn_min_MINI)
                    ...
                    else
                    {
                        ...
                        if (erfc_denom < 0)
                        { // our filtering scale has become too small
                            break;
                        }
...
#pragma omp for reduction(+ : f_coll, f_coll_MINI)
                            ...
                            for (z = 0; z < user_params->HII_DIM; z++)
                            {
                                ...
                                *((float *)deltax_filtered + HII_R_FFT_INDEX(x, y, z)) = fmaxf(
                                ...
                                if (flag_options->USE_MINI_HALOS)
                                {
                                    ...
                                    if (box->Fcoll[counter * HII_TOT_NUM_PIXELS + HII_R_INDEX(x, y, z)] > 1.)
                                        box->Fcoll[counter * HII_TOT_NUM_PIXELS + HII_R_INDEX(x, y, z)] = 1.;
                                    ...
                                    if (box->Fcoll_MINI[counter * HII_TOT_NUM_PIXELS + HII_R_INDEX(x, y, z)] > 1.)
                                        box->Fcoll_MINI[counter * HII_TOT_NUM_PIXELS + HII_R_INDEX(x, y, z)] = 1.;
                ...
                if (flag_options->USE_MASS_DEPENDENT_ZETA)
                {
                    if (f_coll <= f_coll_min)
                        f_coll = f_coll_min;
                    if (flag_options->USE_MINI_HALOS)
                    {
                        if (f_coll_MINI <= f_coll_min_MINI)
                            f_coll_MINI = f_coll_min_MINI;
                    }
                }
                else
                {
                    if (f_coll <= FRACT_FLOAT_ERR)
                        f_coll = FRACT_FLOAT_ERR;
                }
                ...
                if (global_params.FIND_BUBBLE_ALGORITHM != 2 && global_params.FIND_BUBBLE_ALGORITHM != 1)
                { // center method
                    ...
                    for (x = 0; x < user_params->HII_DIM; x++)
                    {
                        for (y = 0; y < user_params->HII_DIM; y++)
                        {
                            for (z = 0; z < user_params->HII_DIM; z++)
                            {
                                ...
                                if ((f_coll * ION_EFF_FACTOR + f_coll_MINI * ION_EFF_FACTOR_MINI > (1. - xHII_from_xrays) * (1.0 + rec)))
                                { // IONIZED!!
                    ...
                    for (z = 0; z < user_params->HII_DIM; z++)
                    {
                        if (isfinite(box->temp_kinetic_all_gas[HII_R_INDEX(x, y, z)]) == 0)
                        {
                            LOG_ERROR("Tk after fully ioinzation is either infinite or a Nan. Something has gone wrong "
                                      ...
                                      box->z_re_box[HII_R_INDEX(x, y, z)], redshift, curr_dens);
                            //                        Throw(ParameterError);
                            Throw(InfinityorNaNError);
                        }
                    }
        ...
        else
        {
            ...
            if (MINI_HALOS)
            {
                if (dens_type == 1)
                {
                    ...
                    *Splined_Fcoll_MINI = (log10_Nion_spline_MINI[overdense_int + NSFR_low * log10_Mturnover_MINI_int] * (1 + (float)overdense_int - dens_val) +
                ...
                if (dens_type == 2)
                {
                    *Splined_Fcoll = (prev_log10_Nion_spline[overdense_int + NSFR_low * log10_Mturnover_int] * (1 + (float)overdense_int - dens_val) +
                    ...
                    *Splined_Fcoll_MINI = (prev_log10_Nion_spline_MINI[overdense_int + NSFR_low * log10_Mturnover_MINI_int] * (1 + (float)overdense_int - dens_val) +
        ...
        if (curr_dens < 0.99 * Deltac)
        {
            ...
            if (MINI_HALOS)
            {
                if (dens_type == 1)
                {
                    ...
                    *Splined_Fcoll_MINI = (Nion_spline_MINI[overdense_int + NSFR_high * log10_Mturnover_MINI_int] * (1 + (float)overdense_int - dens_val) +
                ...
                if (dens_type == 2)
                {
                    *Splined_Fcoll = (prev_Nion_spline[overdense_int + NSFR_high * log10_Mturnover_int] * (1 + (float)overdense_int - dens_val) +
                    ...
                    *Splined_Fcoll_MINI = (prev_Nion_spline_MINI[overdense_int + NSFR_high * log10_Mturnover_MINI_int] * (1 + (float)overdense_int - dens_val) +
