...
float ComputeFullyIoinizedTemperature(float z_re, float z, float delta)
{
    ...
    else
    {
        ...
        if (delta_re <= -1)
            delta_re = -1. + global_params.MIN_DENSITY_LOW_LIMIT;
        // evolving ionized box eq. 6 of McQuinn 2015, ignored the dependency of density at ionization
        if (delta <= -1)
            delta = -1. + global_params.MIN_DENSITY_LOW_LIMIT;
            ...
            for (n_y = 0; n_y < dimension; n_y++)
            {
                ...
                for (n_z = 0; n_z <= midpoint; n_z++)
                {
                    ...
                    { // gaussian
                        ...
                        if (RES == 1)
                        {
                            box[HII_C_INDEX(n_x, n_y, n_z)] *= pow(E, -kR / 2.0);
                        }
                        if (RES == 0)
                        {
                            box[C_INDEX(n_x, n_y, n_z)] *= pow(E, -kR / 2.0);
                        }
                    }
                    else
                    {
...
double dicke(double z)
{
    ...
    if (fabs(cosmo_params_ufunc->OMm - 1.0) < tiny)
    { // OMm = 1 (Einstein de-Sitter)
        return 1.0 / (1.0 + z);
    }
    ...
    else if ((global_params.OMtot < (1 + tiny)) && (fabs(cosmo_params_ufunc->OMl) < tiny))
    { // open, zero lambda case (peebles, pg. 53)
...
double alpha_A(double T)
{
    ...
    ans = pow(E, -28.6130338 - 0.72411256 * logT - 2.02604473e-2 * pow(logT, 2) - 2.38086188e-3 * pow(logT, 3) - 3.21260521e-4 * pow(logT, 4) - 1.42150291e-5 * pow(logT, 5) + 4.98910892e-6 * pow(logT, 6) + 5.75561414e-7 * pow(logT, 7) - 1.85676704e-8 * pow(logT, 8) - 3.07113524e-9 * pow(logT, 9));
    return ans;
}
...
double HeI_ion_crosssec(double nu)
{
    ...
    return 9.492e-16 * ((x - 1) * (x - 1) + 2.039 * 2.039) *
           pow(y, (0.5 * 3.188 - 5.5)) * pow(1.0 + sqrt(y / 1.469), -3.188);
}
...
double HeII_ion_crosssec(double nu)
{
    ...
    return (6.3e-18) / Z / Z * pow(HeII_NUIONIZATION / nu, 4) * pow(E, 4 - (4 * atan(epsilon) / epsilon)) / (1 - pow(E, -2 * PI / epsilon));
}
...
double HI_ion_crosssec(double nu)
{
    ...
    return (6.3e-18) / Z / Z * pow(NUIONIZATION / nu, 4) * pow(E, 4 - (4 * atan(epsilon) / epsilon)) / (1 - pow(E, -2 * PI / epsilon));
}
...
double tau_e(float zstart, float zend, float *zarry, float *xHarry, int len)
{
    ...
    if (zend > global_params.Zreion_HeII)
    { // && (zstart < Zreion_HeII)){
