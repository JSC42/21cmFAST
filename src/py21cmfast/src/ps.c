...
#define FMAX(a, b) (mnarg1 = (a), mnarg2 = (b), (mnarg1) > (mnarg2) ? (mnarg1) : (mnarg2))

static double mnarg1, mnarg2;
#define FMIN(a, b) (mnarg1 = (a), mnarg2 = (b), (mnarg1) < (mnarg2) ? (mnarg1) : (mnarg2))
...
double *log10phi_MINI;
*M_uv_z_MINI, *M_h_z_MINI;
...
double dsigma_dk(double k, void *params)
{
    ...
    else if (user_params_ps->POWER_SPECTRUM == 1)
    { // BBKS
    ...
    else if (user_params_ps->POWER_SPECTRUM == 2)
    { // Efstathiou,G., Bond,J.R., and White,S.D.M., MNRAS,258,1P (1992)
        gamma = 0.25;
    ...
    else if (user_params_ps->POWER_SPECTRUM == 3)
    { // Peebles, pg. 626
        gamma = cosmo_params_ps->OMm * cosmo_params_ps->hlittle * pow(E, -(cosmo_params_ps->OMb) - (cosmo_params_ps->OMb / cosmo_params_ps->OMm));
    ...
    else if (user_params_ps->POWER_SPECTRUM == 4)
    ...
    {                          // output of CLASS
        ...
        p = pow(k, cosmo_params_ps->POWER_INDEX) * T * T;
        if (user_params_ps->USE_RELATIVE_VELOCITIES)
    ...
    if ((global_params.FILTER == 0) || (sigma_norm < 0))
    { // top hat
        if ((kR) < 1.0e-4)
        {
            w = 1.0;
    ...
    else if (global_params.FILTER == 1)
    { // gaussian of width 1/R
        w = pow(E, -kR * kR / 2.0);
    }
    else
    {
...
double dsigmasq_dm(double k, void *params)
{
    ...
    { // top hat
        ...
        {
            dwdr = 9 * cos(kR) * k / pow(kR, 3) + 3 * sin(kR) * (1 - 3 / (kR * kR)) / (kR * Radius);
        }
...
double dsigmasqdm_z0(double M)
{
    ...
    if (user_params_ps->POWER_SPECTRUM == 5)
    { // for CLASS we do not need to renormalize the sigma integral.
        d2fact = 1.0;
    }
    else
    {
...
float erfcc(float x)
{
    ...
    q = fabs(x);
    t = 1.0 / (1.0 + 0.5 * q);
    ans = t * exp(-q * q - 1.2655122 + t * (1.0000237 + t * (0.374092 + t * (0.0967842 + t * (-0.1862881 + t * (0.2788681 + t * (-1.13520398 + t * (1.4885159 + t * (-0.82215223 + t * 0.17087277)))))))));
...
double FgtrM_bias(double z, double M, double del_bias, double sig_bias)
{
    ...
    if (!(sig_bias < sigsmallR))
    {   // biased region is smaller that halo!
              ...
              struct FlagOptions *flag_options, int component, int NUM_OF_REDSHIFT_FOR_LF, float *z_LF, float *M_TURNs, double *M_uv_z, double *M_h_z, double *log10phi)
{
    ...
    */
    int status;
    Try
    { // This try block covers the whole function.
        ...
        for (i_z = 0; i_z < NUM_OF_REDSHIFT_FOR_LF; i_z++)
        {
            ...
            for (i = 0; i < nbins; i++)
            {
                ...
                if (i_unity < 0)
                { // Find the array number at which Fstar crosses unity.
                    if (astro_params->ALPHA_STAR > 0.)
                    {
                        if ((1. - Fstar) < FRACT_FLOAT_ERR)
                            i_unity = i;
                    }
                    else if (astro_params->ALPHA_STAR < 0. && i < nbins - 1)
                    {
                        ...
                        if (Fstar_temp < 1. && (1. - Fstar) < FRACT_FLOAT_ERR)
                            i_unity = i;
                    }
                }
                ...
                if (isinf(Muv_param[i]) || isnan(Muv_param[i]))
                    Muv_param[i] = 10.;
...
double Nion_ConditionalM_MINI(double growthf, double M1, double M2, double sigma2, double delta1, double delta2, double MassTurnover, double MassTurnover_upper, double Alpha_star, double Alpha_esc, double Fstar10, double Fesc10, double Mlim_Fstar, double Mlim_Fesc, bool FAST_FCOLL_TABLES)
{
    ...
    if (FAST_FCOLL_TABLES)
    { // JBM: Fast tables. Assume sharp Mturn, not exponential cutoff.
...
double Nion_ConditionalM(double growthf, double M1, double M2, double sigma2, double delta1, double delta2, double MassTurnover, double Alpha_star, double Alpha_esc, double Fstar10, double Fesc10, double Mlim_Fstar, double Mlim_Fesc, bool FAST_FCOLL_TABLES)
{
    ...
    if (FAST_FCOLL_TABLES && global_params.USE_FAST_ATOMIC)
    { // JBM: Fast tables. Assume sharp Mturn, not exponential cutoff.
...
float GaussLegendreQuad_Nion_MINI(int Type, int n, float growthf, float M2, float sigma2, float delta1, float delta2, float MassTurnover, float MassTurnover_upper, float Alpha_star, float Alpha_esc, float Fstar7_MINI, float Fesc7_MINI, float Mlim_Fstar_MINI, float Mlim_Fesc_MINI, bool FAST_FCOLL_TABLES)
{
    ...
    { // JBM: Fast tables. Assume sharp Mturn, not exponential cutoff.
        ...
        if (nu_higher_limit <= nupivot2)
        { // if both are below pivot2 don't bother adding and subtracting the high contribution
...
float GaussLegendreQuad_Nion(int Type, int n, float growthf, float M2, float sigma2, float delta1, float delta2, float MassTurnover, float Alpha_star, float Alpha_esc, float Fstar10, float Fesc10, float Mlim_Fstar, float Mlim_Fesc, bool FAST_FCOLL_TABLES)
{
    ...
    { // JBM: Fast tables. Assume sharp Mturn, not exponential cutoff.
        ...
        // // 3PLs
        double fcollres = 0.0;
        if (nu_lower_limit >= nucrit_sigma2)
        { // fully in the flat part of sigma(nu), M^alpha is nu-independent.
            return 1e-40;
        }
        else
        { // we subtract the contribution from high nu, since the HMF is set to 0 if sigma2>sigma1
...
#pragma omp parallel shared(log10_Nion_spline, growthf, Mmax, sigma2, log10_overdense_spline_SFR, Mturns, Mturns_MINI,                                     \
                                ...
                                Fstar7_MINI, Fesc7_MINI, Mlim_Fstar_MINI, Mlim_Fesc_MINI) private(i, j) num_threads(user_params_ps->N_THREADS)
    {
#pragma omp for
        for (i = 0; i < NSFR_low; i++)
        {
...
#pragma omp parallel shared(prev_Nion_spline, growthf, Mmin, Mmax, sigma2, prev_Overdense_spline_SFR, Mturns,                                       \
                                ...
                                Mcrit_atom, Fstar7_MINI, Fesc7_MINI, Mlim_Fstar_MINI, Mlim_Fesc_MINI) private(i, j) num_threads(user_params_ps->N_THREADS)
    {
#pragma omp for
        for (i = 0; i < NSFR_high; i++)
        {
...
#pragma omp parallel shared(z_val, Nion_z_val, Nbin, zmin, zmax, Mmin, Alpha_star, Alpha_star_mini, Alpha_esc, Fstar10, Fesc10, Mlim_Fstar, Mlim_Fesc, \
                                Nion_z_val_MINI, MassTurnover, Fstar7_MINI, Fesc7_MINI, Mlim_Fstar_MINI, Mlim_Fesc_MINI) private(i, j, Mcrit_atom_val) num_threads(user_params_ps->N_THREADS)
    {
#pragma omp for
        for (i = 0; i < Nbin; i++)
        {
...
#pragma omp parallel shared(z_X_val, zmin, zmax, Nbin, SFRD_val, Mmin, Alpha_star, Alpha_star_mini, Fstar10, Mlim_Fstar, \
                                SFRD_val_MINI, MassTurnover, Fstar7_MINI, Mlim_Fstar_MINI) private(i, j, Mcrit_atom_val) num_threads(user_params_ps->N_THREADS)
    {
#pragma omp for
        for (i = 0; i < Nbin; i++)
        {
...
#pragma omp parallel shared(log10_SFRD_z_low_table, growthf, Mmax, sigma2, overdense_low_table, Mcrit_atom, Alpha_star, Alpha_star_mini, Fstar10, Mlim_Fstar, \
                                log10_SFRD_z_low_table_MINI, MassTurnover, Fstar7_MINI, Mlim_Fstar_MINI, ln_10) private(i, k) num_threads(user_params_ps->N_THREADS)
        {
#pragma omp for
            for (i = 0; i < NSFR_low; i++)
            {
...
#pragma omp parallel shared(SFRD_z_high_table, growthf, Mmin, Mmax, sigma2, overdense_high_table, Mcrit_atom, Alpha_star, Alpha_star_mini, Fstar10, \
                                Mlim_Fstar, SFRD_z_high_table_MINI, MassTurnover, Fstar7_MINI, Mlim_Fstar_MINI) private(i, k) num_threads(user_params_ps->N_THREADS)
        {
#pragma omp for
            for (i = 0; i < NSFR_high; i++)
            {
    ...
    float *stored_redshift, float *absolute_delta_z)
{
    ...
    if (*redshift < global_params.PhotonConsEndCalibz)
    {
        LOG_ERROR(
            ...
            *redshift, global_params.PhotonConsEndCalibz, *redshift);
        //        Throw(ParameterError);
        Throw(PhotonConsError);
    }
